...
# Code Template V 1.1
# URL: https://au2mator.com/move-user-in-ou-active-directory-self-service-with-au2mator/
...
if ($ErrorCount -eq 0) {
    $au2matorReturn = "USer $c_User moved"
    $AdditionalHTML = "<br>
        User " + (Get-ADUser -identity $c_User).DisplayName + " has been moved
        <br>
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to move User $c_User, Error: $Error"
    $Status = "ERROR"
}
